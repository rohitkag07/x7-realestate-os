  ...
  return (
    <>
      <PageHeader
        title="Conversions"
        titleHi="कन्वर्ज़न"
        description="Qualified leads, payment intent, booking actions, and owner handoff for the selected vertical."
        actions={<Button variant="outline" size="sm"><Download className="h-4 w-4 mr-2" /> Export Report</Button>}
      />
      <p className="mb-6 -mt-3 text-xs text-muted-foreground">{salesReadSourceLabel(source)}</p>

      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <KPICard label="Total Conversions" value={bookings.length} icon={IndianRupee} accent="success" />
        <KPICard label="Token Collected" value={formatINR(totalRevenue)} icon={IndianRupee} accent="success" />
        <KPICard label="Available Slots" value={plots.filter((plot) => plot.status === 'available').length} icon={IndianRupee} accent="primary" />
