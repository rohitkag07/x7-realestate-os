...
export async function POST(req: NextRequest) {
  ...
  const { data: lead, error } = await (sb.from('leads') as any).insert({
  ...
  if (date) {
    await (sb.from('site_visits') as any).insert({ lead_id: lead.id, project_id, scheduled_date: date, scheduled_time: '14:00', status: 'scheduled' });
  }
  // Queue a CAPI Lead event for Meta attribution
  await (sb.from('capi_events') as any).insert({ builder_id, lead_id: lead.id, event_name: 'Lead', user_data: { phone: normalized, email: email ?? null, country: 'IN' }, custom_data: { source } });
