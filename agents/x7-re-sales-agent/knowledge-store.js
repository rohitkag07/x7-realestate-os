export async function fetchPublishedKnowledge({ supabase, businessId, playbookId }) {
  ...
  let query = supabase
    ...
    .select('id, business_id, playbook_id, type, title, question, content, keywords, locale, status, okf_slug, source_type, source_url, media_url, metadata, is_active, version, last_reviewed_at')
